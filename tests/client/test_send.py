...
def test_send_multichannel_message_failure(
    ...
    with pytest.raises(SuomiFiAPIError, match="Failed to send multichannel message"):
        client.send_multichannel_message(
            title="Test",
            body="Test",
            body_format=BodyFormat.TEXT,
...
def test_send_paper_mail_without_id_failure(
    ...
    with pytest.raises(SuomiFiAPIError, match="Failed to send paper mail without id"):
        client.send_paper_mail_without_id(
            recipient_address=recipient_address,
            sender_address=sender_address,
            attachment_id="att_1",
